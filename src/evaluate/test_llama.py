...
def response_round(round_index, query, tokenizer, model):
    ...
    response_task = cut_pattern(count_index=(round_index + 1) * 2,
                                ...
                                pattern='INST]')
    print('Psychologist: ' + response_task)
    return response_task


def rounds(prompt, tokenizer, model):
    query0 = ("<s>[INST] <<SYS>> \nYou are a psychologist. \n<</SYS>>"
              + prompt + '[/INST]')
    response0 = response_round(round_index=0, query=query0,
                               tokenizer=tokenizer, model=model)
    return response0


def ask_llama_rounds(MODEL_PATH):
    tokenizer = LlamaTokenizer.from_pretrained(MODEL_PATH)
    model = LlamaForCausalLM.from_pretrained(MODEL_PATH, device_map='auto')
    print("Please input your thought:")
    p_r1 = input()
    prompt1 = (
    ...
    rounds(prompt=prompt1, tokenizer=tokenizer, model=model)
    ...
    rounds(prompt=prompt2, tokenizer=tokenizer, model=model)
    ...
    rounds(prompt=prompt3, tokenizer=tokenizer, model=model)


def generate_round1_llama(MODEL_PATH):
    ...
    for i in range(len(patient_round1_list)):
        ...
        df1 = pd.DataFrame({'index': [i],
                            ...
                            'd_round1': [rounds(prompt=prompt1, tokenizer=tokenizer, model=model)],
                            'd_round2': [rounds(prompt=prompt2, tokenizer=tokenizer, model=model)],
                            'd_round3': [rounds(prompt=prompt3, tokenizer=tokenizer, model=model)]})
        # df1.to_csv('data/gpt4_generation/511_gpt4_generation_851_3000.csv', mode='a', index=False, header=False)
        df1.to_csv('llama_rounds.csv', mode='a', index=False, header=False)


if __name__ == '__main__':
    # generate_round1_llama()
    ...
    # ask_llama_rounds(p_r1, p_r3)
    MODEL_PATH = 'meta-llama/Llama-2-7b-chat-hf'
    ask_llama_rounds(MODEL_PATH)
